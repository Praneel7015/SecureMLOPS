...
def submit_training_job(dataset_id: str, dataset_dir: str, config: dict[str, Any]) -> dict[str, Any]:
    with _lock:
        ...
        job = init_job(job_id, dataset_id, config)

        def _runner():
            update_job(job_id, status="running")
            run_training_job(job_id, dataset_dir, config)
